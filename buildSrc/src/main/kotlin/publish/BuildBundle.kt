...
abstract class BuildBundle : DefaultTask() {
    @get:OutputFile
    val bundlePath: File =
        (Paths.get(project.layout.buildDirectory.asFile.get().toString()).resolve("libs")
            .resolve("extension.bundle")).toFile()
    ...
    fun partition(name: String, configure: Action<PartitionConfiguration>) {
        ...
        val collection = project.files(configValues.artifacts.flatMap { it.outputs.files })
